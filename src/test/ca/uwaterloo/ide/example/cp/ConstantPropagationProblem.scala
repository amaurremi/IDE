...
class ConstantPropagationProblem extends IdeProblem {
   ...
   */
  override def initialSeeds: Seq[(XEdge, CpMicroFunction)] = ???

  override type MicroFunction = CpMicroFunction
  override type LatticeElem = CpLatticeElem
  override val Bottom: LatticeElem = ???
  override val Top: LatticeElem = ???
  override val Id: MicroFunction = ???
  override val λTop: MicroFunction = ???

  trait CpMicroFunction extends MicroFunctionI
  trait CpLatticeElem extends LatticeElementI[CpLatticeElem]
